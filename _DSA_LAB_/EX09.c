...
{
    CircularQueue *q =
        (CircularQueue *)malloc(sizeof(CircularQueue));
...
void enqueue(CircularQueue *q, int data)
{
    ...
    if (isEmpty(q))
    {
        ...
        newNode
            ->next = newNode; // Point to itself(circularreference)
    }
    else
    {
        newNode
...
-1;
 ...
 ->front
->data; }
// Function to get th rear element
int getRear(CircularQueue* q) {
 if (isEmpty(q)) {
 printf("Queue isempty.\n");
 return
